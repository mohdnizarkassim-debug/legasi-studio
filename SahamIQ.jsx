...
// ============================================================
// CONFIGURATION — Tukar API key kau kat sini
// ============================================================
const API_KEY = import.meta.env.VITE_TWELVE_DATA_KEY || "YOUR_API_KEY_HERE";
...
// ============================================================
const MY_PORTFOLIO = [
  { code: "MAYBANK",  exchange: "KLSE", unit: 1000, beliPrice: 9.20,  name: "Malayan Banking Bhd",   sector: "Banking"   },
  { code: "PBBANK",   exchange: "KLSE", unit: 2000, beliPrice: 4.10,  name: "Public Bank Bhd",        sector: "Banking"   },
  { code: "CIMB",     exchange: "KLSE", unit: 1500, beliPrice: 7.50,  name: "CIMB Group Holdings",    sector: "Banking"   },
  { code: "TENAGA",   exchange: "KLSE", unit: 500,  beliPrice: 11.00, name: "Tenaga Nasional Bhd",    sector: "Utilities" },
];

// ============================================================
// FALLBACK DATA — Guna kalau API tak available / limit habis
// ============================================================
const FALLBACK = {
  MAYBANK: { price: 9.52,  change: 0.12,  pct: 1.28,  pe: 12.3, dy: 5.8, roe: 11.2, high: 10.20, low: 8.60,  volume: "12.3M", cap: "RM107.2B" },
  PBBANK:  { price: 4.21,  change: -0.03, pct: -0.71, pe: 13.1, dy: 4.2, roe: 13.5, high: 4.80,  low: 3.90,  volume: "8.7M",  cap: "RM81.6B"  },
  CIMB:    { price: 7.85,  change: 0.15,  pct: 1.95,  pe: 10.8, dy: 5.1, roe: 10.3, high: 8.20,  low: 6.50,  volume: "18.1M", cap: "RM82.1B"  },
  TENAGA:  { price: 11.30, change: 0.08,  pct: 0.71,  pe: 15.6, dy: 3.9, roe: 9.8,  high: 12.50, low: 10.10, volume: "5.2M",  cap: "RM64.5B"  },
};

// ============================================================
// API FUNCTIONS
// ============================================================
async function fetchQuote(symbol, exchange = "KLSE") {
  try {
    const res = await fetch(`${BASE_URL}/quote?symbol=${symbol}&exchange=${exchange}&apikey=${API_KEY}`);
    const data = await res.json();
    if (data.status === "error" || !data.close) throw new Error("API error");
    return {
      price:  parseFloat(data.close),
      change: parseFloat(data.change),
      pct:    parseFloat(data.percent_change),
      high:   parseFloat(data.fifty_two_week?.high || data.high),
      low:    parseFloat(data.fifty_two_week?.low  || data.low),
      volume: data.volume,
      name:   data.name,
    };
  } catch {
...
async function fetchCandles(symbol, exchange = "KLSE") {
  try {
    const res = await fetch(`${BASE_URL}/time_series?symbol=${symbol}&exchange=${exchange}&interval=1day&outputsize=30&apikey=${API_KEY}`);
    const data = await res.json();
    if (data.status === "error" || !data.values) throw new Error();
      ...
      <svg width="100%" height="40" viewBox="0 0 200 24" preserveAspectRatio="none">
        <defs>
          <linearGradient id={`mg${positive}`} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={positive ? "#059669" : "#DC2626"} stopOpacity="0.18" />
...
export default function SahamIQ() {
  ...
  const [watchlist, setWatchlist] = useState(["MAYBANK", "CIMB"]);
  ...
  const loadData = useCallback(async () => {
    ...
    for (const s of MY_PORTFOLIO) {
      const q = await fetchQuote(s.code, s.exchange);
      if (q) {
    ...
    // Load candles selepas quotes
    for (const s of MY_PORTFOLIO) {
      const c = await fetchCandles(s.code, s.exchange);
    ...
    <div style={{ fontFamily: "'Sora', sans-serif", background: "#F0F4F8", minHeight: "100vh", color: "#0F172A" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input[type=range] { accent-color: #0F4C81; }
      `}</style>
      ...
      <header style={{ background: "#fff", borderBottom: "1px solid #E2E8F0", padding: "0 20px", height: 58, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <button onClick={loadData} style={{ width: 34, height: 34, borderRadius: 8, border: "1px solid #E2E8F0", background: "#F8FAFC", cursor: "pointer", fontSize: 14 }} title="Refresh">↻</button>
                  ...
                  return (
                    <div key={s.code} className="card" onClick={() => goDetail(s.code)}
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                        <div>
                          <div style={{ fontSize: 14, fontWeight: 800 }}>{s.code}</div>
              ...
              <>
                    ...
                    <div key={s.code} className="row-hover" style={{ padding: "16px 20px", borderBottom: i < portfolioStats.length - 1 ? "1px solid #F1F5F9" : "none", cursor: "pointer" }} onClick={() => goDetail(s.code)}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                          <div style={{ width: 38, height: 38, borderRadius: 10, background: "#EFF6FF", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: "#0F4C81" }}>{s.code.slice(0, 2)}</div>
                          <div>
                            <div style={{ fontSize: 14, fontWeight: 700 }}>{s.code}</div>
                      ...
                      <div style={{ background: "#F1F5F9", borderRadius: 4, height: 5, overflow: "hidden" }}>
                        <div style={{ width: `${Math.min(Math.abs(s.plPct) * 4, 100)}%`, height: "100%", background: s.pl >= 0 ? "#059669" : "#DC2626", borderRadius: 4, transition: "width 0.8s ease" }} />
                      </div>
                ...
                <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #E2E8F0", padding: "18px 20px", marginTop: 14, boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                  ...
                  {portfolioStats.map(s => {
                    ...
                    const colors = { MAYBANK: "#0F4C81", PBBANK: "#059669", CIMB: "#D97706", TENAGA: "#7C3AED" };
                    return (
            ...
            {watchlist.length === 0 ? (
              ...
              <div style={{ display: "grid", gap: 10 }}>
                {MY_PORTFOLIO.filter(s => watchlist.includes(s.code)).map(s => {
                  ...
                  if (!live) return null;
                  return (
                    <div key={s.code} style={{ background: "#fff", borderRadius: 14, padding: "16px 18px", border: "1px solid #E2E8F0", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}
                      onClick={() => goDetail(s.code)}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <div style={{ width: 40, height: 40, borderRadius: 10, background: "#EFF6FF", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, color: "#0F4C81" }}>{s.code.slice(0, 2)}</div>
                        <div>
                          <div style={{ fontSize: 14, fontWeight: 700 }}>{s.code}</div>
                ...
                return (
                  <div key={s.code} className="row-hover" style={{ background: "#fff", borderRadius: 14, padding: "16px 20px", border: "1px solid #E2E8F0", cursor: "pointer", boxShadow: "0 1px 3px rgba(0,0,0,0.04)", transition: "all 0.15s" }} onClick={() => goDetail(s.code)}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <div style={{ width: 42, height: 42, borderRadius: 11, background: "#EFF6FF", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, color: "#0F4C81" }}>{s.code.slice(0, 2)}</div>
                        <div>
                          <div style={{ fontSize: 14, fontWeight: 800 }}>{s.code} <span style={{ fontSize: 11, color: "#94A3B8", fontWeight: 400 }}>· {s.sector}</span></div>
                          <div style={{ fontSize: 12, color: "#64748B" }}>{s.name}</div>
                        </div>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                        <div style={{ display: "none", gap: 20 }} className="desktop-stats">
                          {[["PE", live.pe + "x"], ["DY", live.dy + "%"], ["ROE", live.roe + "%"]].map(([l, v]) => (
                            <div key={l} style={{ textAlign: "center" }}>
                              <div style={{ fontSize: 10, color: "#94A3B8" }}>{l}</div>
                              <div style={{ fontSize: 13, fontWeight: 700, fontFamily: "'DM Mono',monospace" }}>{v}</div>
                            </div>
                          ))}
                        </div>
                        <div style={{ textAlign: "right" }}>
          ...
          <div className="card">
            ...
            <div style={{ background: "#fff", borderRadius: 14, padding: "22px", border: "1px solid #E2E8F0", marginBottom: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  <div style={{ width: 48, height: 48, borderRadius: 12, background: "#EFF6FF", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 800, color: "#0F4C81" }}>{selected.code.slice(0, 2)}</div>
                  <div>
                    <h1 style={{ fontSize: 20, fontWeight: 800, letterSpacing: "-0.5px" }}>{selected.code}</h1>
                ...
                <div style={{ textAlign: "right" }}>
                  ...
                  <div style={{ fontSize: 14, color: selected.pct >= 0 ? "#059669" : "#DC2626", fontWeight: 700 }}>
                    {selected.pct >= 0 ? "▲" : "▼"} RM {Math.abs(selected.change || 0).toFixed(2)} ({Math.abs(selected.pct || 0).toFixed(2)}%)
                  </div>
                </div>
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(130px,1fr))", gap: 10, marginBottom: 14 }}>
              {[
                ...
                <div key={l} style={{ background: "#fff", borderRadius: 10, padding: "14px 16px", border: "1px solid #E2E8F0", boxShadow: "0 1px 2px rgba(0,0,0,0.03)" }}>
            ...
            {/* Portfolio position for this stock */}
            {(() => {
      ...
      <footer style={{ textAlign: "center", padding: "24px 16px", fontSize: 11, color: "#94A3B8" }}>
        SahamIQ · Data via Twelve Data · {usingFallback ? "Demo mode — masukkan API key untuk data sebenar" : "Data realtime KLSE"}
      </footer>
    </div>
  );
}
